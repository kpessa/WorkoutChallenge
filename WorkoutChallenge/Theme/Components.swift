...
struct Chip: View {
    ...
    var body: some View {
        ...
        .overlay(
            Capsule().stroke(isOn ? Color.clear : Color.appBorder, lineWidth: 1)
        )
...
/// A single square in the challenge's 90-day grid. Four states:
/// - completed: solid Volt fill
/// - proposed:  Volt dashed outline, empty
/// - upcoming:  muted surface2 fill, tertiary text
/// - today:     solid ink fill with Volt outline + Volt number
struct DayCell: View {
    enum State { case completed, proposed, upcoming, today }
    let day: Int
    let state: State
            ...
            RoundedRectangle(cornerRadius: 6)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 6)
                        .stroke(stroke,
                                style: StrokeStyle(
                                    lineWidth: state == .proposed || state == .today ? 2 : 1,
                                    dash: state == .proposed ? [3, 2] : []))
                )
            Text("\(day)")
    ...
    private var fill: Color {
        ...
        case .proposed:  return .clear
        case .upcoming:  return .appSurface2
        case .today:     return inkOnVolt
        }
    }
    private var stroke: Color {
        switch state {
        case .completed: return .accentVolt
        case .proposed:  return .accentVolt
    ...
    private var textColor: Color {
        switch state {
        case .completed: return inkOnVolt
        case .proposed:  return .accentVolt
...
struct SigmoidCurve: View {
    ...
    var strokeColor: Color = .accentVolt
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            if showMilestones {
                // Milestones at t = 0.23, progress, 0.92
                Circle().fill(strokeColor).frame(width: 7, height: 7)
                    .position(pt(0.23))
                Circle().fill(strokeColor)
                    .overlay(Circle().stroke(Color.appBg, lineWidth: 2))
                    .frame(width: 10, height: 10)
                    .position(pt(max(0, min(1, progress))))
                Circle().fill(strokeColor).opacity(0.4).frame(width: 7, height: 7)
                    .position(pt(0.92))
            }
        }
    }
